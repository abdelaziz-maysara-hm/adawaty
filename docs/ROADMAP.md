...
### Performance (mobile Core Web Vitals -- 0.5.149 + 0.5.150)
...
- **Re-measured after deploying, confirming the fix worked**: mobile performance went from 42 to
  **78** on the same URL. The largest remaining finding was "Render-blocking requests -- Est
  savings of 320ms" (separate `main.css`/`product.css` requests) -- fixed in 0.5.150 by merging
  them into a single generated `site.css` at build time. Not yet re-measured post-0.5.150 deploy.
- While fixing the JS bundle-size issue, 4 previously-hidden duplicate tool ids were found and
  resolved (`percentage-calculator`, `discount-calculator`, `age-calculator`, `bmi-calculator`) --
  worth keeping in mind that `tool-definitions.js` may still be worth a periodic audit for other
