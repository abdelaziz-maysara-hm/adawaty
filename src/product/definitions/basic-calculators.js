/**
 * "Basic calculators" -- 20 tools that were previously defined inline
 * directly inside tool-definitions.js itself (BMI, age, percentage,
 * VAT, loan, compound interest, date difference, BMR/TDEE/ideal-weight/
 * water-intake/body-surface-area, grade/GPA/average/weighted-average/
 * attendance, percentage-change, ratio), extracted into their own file
 * here to fix a real, measured performance problem: every tool page
 * loaded ALL 628 tools' code (~1.7 MB combined across 123 files) just
 * to render one tool, confirmed via a live PageSpeed Insights report
 * showing 42/100 mobile performance (vs. 98/100 desktop) with "Reduce
 * unused JavaScript -- Est savings of 255 KiB" as a specific finding.
 * Each tool page now dynamically imports only its own definitions
 * file at runtime (see tool-page.js and generate-product-pages.mjs's
 * per-page manifest) -- these 20 tools needed a real file to import
 * rather than being unreachable except through the full aggregator.
 */
...
const basicCalculatorToolDefinitions = Object.freeze({
...
export { basicCalculatorToolDefinitions };
